...
export default function Blog() {
  ...
  return <Layout><div className="relative overflow-hidden bg-[radial-gradient(circle_at_15%_10%,hsl(var(--accent)/0.10),transparent_28%)]">
    ...
    <section className="mx-auto max-w-[1320px] px-4 pb-20 pt-14 sm:px-7"><div className="mb-7 flex flex-wrap items-end justify-between gap-6"><div><p className="mb-2 text-xs font-semibold uppercase text-accent">{copy.guides}</p><h2 className="font-heading text-3xl font-semibold text-primary">{copy.latest}</h2></div><div className="flex flex-wrap gap-2"><Filter active={selected === 'all'} onClick={() => setSelected('all')}>{copy.all}</Filter>{(categoriesQuery.data || []).map((category) => <Filter key={category.id} active={selected === category.slug} onClick={() => setSelected(category.slug)}>{local(category, 'name', language)}</Filter>)}</div></div>
      {postsQuery.isLoading ? <Status>{copy.loading}</Status> : postsQuery.isError ? <Status error>{copy.error}</Status> : !visible.length ? <Status>{copy.empty}</Status> : <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{visible.map((post) => <Link to={`/blog/${post.slug}`} key={post.id} className="group flex overflow-hidden rounded-2xl border bg-white/75 p-4 shadow-soft md:flex-col"><div className="w-32 shrink-0 overflow-hidden rounded-lg bg-secondary md:w-full"><img src={imageUrl(post.cover_image_url) || '/placeholder.svg'} alt={local(post, 'cover_image_alt', language) || local(post, 'title', language)} className="aspect-square h-full w-full object-cover group-hover:scale-[1.03] md:aspect-[4/3]"/></div><div className="flex flex-1 flex-col px-4 py-1 md:px-1 md:pt-5"><div className="mb-2 text-[11px] font-semibold uppercase text-accent">{local(post, 'category_name', language)}</div><h3 className="font-heading text-xl font-semibold text-primary">{local(post, 'title', language)}</h3><p className="mt-3 hidden flex-1 text-sm leading-6 text-muted-foreground sm:block">{local(post, 'excerpt', language)}</p><div className="mt-4 flex justify-between border-t pt-4 text-xs text-muted-foreground"><time>{formatDate(post.published_at, language)}</time><span className="inline-flex items-center gap-1 text-primary">{copy.read}<ArrowRight className="size-3.5"/></span></div></div></Link>)}</div>}
    </section></div></Layout>;
}
